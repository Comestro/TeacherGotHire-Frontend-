...
const TeacherViewPage = () => {
    ...
    <div className="max-w-7xl mx-auto px-4 py-8 animate-pulse">
      ...
      <div className="bg-white rounded-lg shadow">
        <div className="flex border-b p-4">
          {[1, 2, 3].map((item) => (
            <div key={item} className="h-10 bg-gray-200 rounded w-40 mx-2"></div>
          ))}
        </div>
        <div className="p-6">
          {[1, 2, 3, 4].map((item) => (
            <div key={item} className="h-20 bg-gray-200 rounded w-full mb-4"></div>
          ))}
        </div>
      </div>
    </div>
  ...
  if (error) return <div className="max-w-7xl mx-auto px-4 py-8 bg-red-50 text-red-600 rounded-lg shadow p-4">Error: {error}</div>;
  if (!teacher) return <div className="max-w-7xl mx-auto px-4 py-8 bg-yellow-50 text-yellow-700 rounded-lg shadow p-4">Teacher not found</div>;

  return (
    <div className="bg-gray-50 min-h-screen pb-12">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <Link to="/recruiter" className="mb-6 inline-flex items-center text-blue-600 hover:text-blue-800 transition-colors group">
        ...
        {/* Tabs Navigation */}
        <div ref={tabsRef} className={`bg-white rounded-t-xl shadow-md transition-all duration-300 ${stickyTabs ? 'sticky top-0 z-10 shadow-md rounded-none' : ''}`}>
          <div className="flex overflow-x-auto scrollbar-hide">
            <button
              className={`px-4 py-4 sm:px-6 whitespace-nowrap text-sm font-medium flex items-center ${activeTab === 'qualifications'
            ...
            <button
              className={`px-4 py-4 sm:px-6 whitespace-nowrap text-sm font-medium flex items-center ${activeTab === 'experience'
                  ? 'border-b-2 border-teal-500 text-teal-600'
                  : 'text-gray-500 hover:text-gray-700'
                }`}
            ...
            <button
              className={`px-4 py-4 sm:px-6 whitespace-nowrap text-sm font-medium flex items-center ${activeTab === 'skills'
                  ? 'border-b-2 border-teal-500 text-teal-600'
                  : 'text-gray-500 hover:text-gray-700'
                }`}
            ...
            <button
              className={`px-4 py-4 sm:px-6 whitespace-nowrap text-sm font-medium flex items-center ${activeTab === 'preferences'
                  ? 'border-b-2 border-teal-500 text-teal-600'
                  : 'text-gray-500 hover:text-gray-700'
                }`}
                ...
                {teacher.teacherqualifications?.length > 0 ? (
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {teacher.teacherqualifications.map((qualification) => (
                      <div key={qualification.id} className="p-4 bg-gray-50 rounded-lg border border-gray-100 hover:shadow-md transition-shadow">
                  ...
                  <p className="text-gray-500 italic">No qualification information available</p>
                )}
              </div>
            </div>
          )}
              ...
              {teacher.teacherexperiences?.length > 0 ? (
                <div className="space-y-4">
                  {teacher.teacherexperiences.map((experience) => (
                    <div key={experience.id} className="p-4 bg-gray-50 rounded-lg border border-gray-100 hover:shadow-md transition-shadow">
                      ...
                      <p className="mt-2 text-gray-600">{experience.achievements}</p>
                    </div>
                  ))}
                </div>
              ) : (
          ...
          {activeTab === 'skills' && (
            <div>
              ...
              {teacher.teacherskill?.length > 0 ? (
                ...
                <p className="text-gray-500 italic">No skill information available</p>
              )}
            </div>
          )}
          ...
          {activeTab === 'preferences' && (
            <div>
              ...
              {teacher.preferences?.length > 0 ? (
                      ...
                      <div className="p-4 bg-gray-50 rounded-lg">
                        ...
                        <div className="flex flex-wrap gap-2">
                          {preference.prefered_subject?.length > 0 ? (
                            ...
                            <p className="text-gray-500 italic">No preferred subjects listed</p>
                          )}
                        </div>
                      </div>
                      ...
                      <div className="p-4 bg-gray-50 rounded-lg">
                        ...
                        <div className="flex flex-wrap gap-2">
                          {preference.class_category?.length > 0 ? (
                            ...
                            <p className="text-gray-500 italic">No class categories listed</p>
                          )}
                        </div>
                      </div>
                    </div>
                ...
                <p className="text-gray-500 italic">No teaching preferences available</p>
              )}
            </div>
          )}
        </div>
