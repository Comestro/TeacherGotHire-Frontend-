...
const ExamMode = () => {
  ...
  const handleGeneratePasskey = async (event) => {
    ...
    if (selectedCenterId && pass_exam_id) {
      dispatch(
        generatePasskey({ user_id, exam_id, center_id: selectedCenterId }),
      );
      navigate("/teacher");
    } else {
    ...
    <div className="bg-primary flex items-center justify-center px-4 py-8 select-none relative min-h-screen">
      {!isWindowFocused && (
        <div className="fixed inset-0 z-50 bg-black/80 backdrop-blur-md flex items-center justify-center">
          <div className="text-center p-8 bg-white rounded  max-w-md mx-4">
            <div className="text-4xl mb-4">⚠️</div>
            <h3 className="text-xl font-bold text-gray-900 mb-2">
              Security Warning
            </h3>
            <p className="text-gray-600">
              Please keep the window focused. Screen recording and screenshots
              are not allowed.
            </p>
          </div>
        </div>
      )}
      <div className="bg-white shadow-2xl rounded  p-8 w-full max-w-lg border border-gray-200">
        <div className="text-center mb-8">
          <div className="inline-flex items-center justify-center w-16 h-16 bg-primary rounded-full mb-4">
            <span className="text-3xl">📝</span>
          </div>
          <h2 className="text-2xl font-bold text-text mb-2">
            Generate Exam Passkey
          </h2>
          <p className="text-secondary text-sm">
            Select your preferred exam center to generate passkey
          </p>
        </div>
        ...
        <div className="mt-6 p-4 bg-blue-50 border border-blue-200 rounded-lg">
          <p className="text-xs text-blue-800 text-center">
            💡 <strong>Note:</strong> This passkey will be used for offline exam
            verification at the selected center.
          </p>
        </div>
      </div>
    </div>
